...
const initializeBot = (botConfig) => {
  ...
  const client = new Client({
    ...
    qrTimeoutMs: 60000 // Set a timeout for QR code generation, useful for pair code flow
  });
  ...
  botStates[botConfig.id] = {
    ready: false,
    qrCodeData: null,
    pairCode: null, // New field for pairing code
    lastSession: null,
    config: botConfig
  };
  ...
  client.on('ready', () => {
    ...
    botStates[botConfig.id].pairCode = null;
  ...
  client.on('disconnected', reason => {
    ...
    // Reinitialize client to allow re-pairing
    client.destroy();
    client.initialize();
  });

  client.on('change_state', state => {
    console.log(`[STATE_CHANGE] Bot '${botConfig.id}': ${state}`);
    if (state === 'PAIRING') {
      console.log(`[PAIRING] Bot '${botConfig.id}' is in pairing mode, waiting for code.`);
  ...
  client.on('pairing_code', (code) => {
    botStates[botConfig.id].pairCode = code;
    console.log(`[PAIR_CODE] Bot '${botConfig.id}': ${code}`);
  });
  ...
  // No longer need syncInterval for connection state with pair code flow
  // clearInterval(botStates[botConfig.id].syncInterval);
};
...
botsConfig.bots.forEach(botConfig => {
  if (botConfig.enabled) {
    ...
    botStates[botConfig.id] = {
      ready: false,
      qrCodeData: null,
      pairCode: null, // Ensure pairCode is initialized even for disabled bots
      lastSession: null,
      config: botConfig
    };
  }
...
app.get('/bots', (req, res) => {
  const botsList = Object.keys(botStates).map(botId => ({
    ...
    pairCodeAvailable: Boolean(botStates[botId].pairCode),
    pairCode: botStates[botId].pairCode, // Include the actual pair code
...
app.post('/bots/:botId/toggle', async (req, res) => {
  ...
  if (targetEnabled) {
    ...
    botStates[botId].pairCode = null; // Clear pair code on pause
  }
...
app.get('/bots/:botId/status', (req, res) => {
  ...
  res.json({
    ...
    ready: botStates[botId].ready,
    qrAvailable: Boolean(botStates[botId].qrCodeData),
    pairCodeAvailable: Boolean(botStates[botId].pairCode), // New field for pair code status
    lastSession: botStates[botId].lastSession
  });
});

// Request pairing code for a specific bot
app.post('/bots/:botId/pair', async (req, res) => {
  const { botId } = req.params;
  ...
  const client = bots[botId];
  if (!client) {
    return res.status(503).json({ error: `Bot '${botId}' client not initialized` });
  }

  // Check if already connected
  if (botStates[botId].ready) {
    return res.status(400).json({ error: `Bot '${botId}' is already ready. No need to pair.` });
  }
  ...
  try {
    const code = await client.requestPairingCode(phoneNumber.replace(/\s/g, '')); // Remove spaces from phone number
    botStates[botId].pairCode = code; // Store the generated pair code
    console.log(`[PAIR_REQUEST] Bot '${botId}' requested pairing for phone ${phoneNumber}, code: ${code}`);
    res.json({ botId, success: true, code });
  } catch (error) {
    console.error(`[PAIR_ERROR] Bot '${botId}' failed to request pairing code:`, error);
    res.status(500).json({ error: error.message || 'Failed to request pairing code' });
  }
});

// Get QR code for a specific bot (now also handles pair code display)
app.get('/bots/:botId/qr', async (req, res) => {
  ...
  const { qrCodeData, pairCode } = botStates[botId];

  if (pairCode) {
    return res.json({ botId, pairCode, qrAvailable: false, pairCodeAvailable: true });
  }

  if (qrCodeData) {
    const qrDataUrl = await QRCode.toDataURL(qrCodeData);
    return res.json({ botId, qr: qrCodeData, qrDataUrl, qrAvailable: true, pairCodeAvailable: false });
  }
...
app.post('/message', validateApiKey, async (req, res) => {
  ...
  try {
    ...
    console.log(`[DEBUG] Send response:`, JSON.stringify(response, null, 2));
    
    // Handle different response structures
    let messageId = null;
    if (response && response.id) {
      messageId = response.id._serialized || response.id;
    } else if (response && response._serialized) {
    ...
    console.log(`[MESSAGE_OK] Bot '${botId}' to=${chatId} id=${messageId}`);
    res.json({ success: true, botId, id: messageId, to: chatId, message });
  } catch (error) {
...
app.listen(port, () => {
  ...
  console.log(`[SERVER] Available bots: ${Object.keys(botsConfig.bots.filter(b => b.enabled).map(b => b.id)).join(', ')}`);
});
